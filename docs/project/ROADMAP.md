...
## v53.3 play access note

Play is now visible in the Core Student Mode preset so games are discoverable without switching to Full mode. Future work can add per-game staff visibility toggles.


## v54 file-structure note

v54 makes the codebase easier to grow by organizing components by product surface and workflow. Future features should be added to the relevant feature folder instead of returning to a flat components directory.
